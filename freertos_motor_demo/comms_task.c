...
//*****************************************************************************
//
// COMMS transmit loop delay value in ms. COMMS transmit frequency is 1000 divided by this number.
//
//*****************************************************************************
...
volatile char g_pcRead[5]; //max recieve message size is 8 bytes
...
CommsTask(void *pvParameters)
{
    ...
    while(1)
    {
        ...
        if(g_pcRead[0] == 0x00){              //Request Message - prep response
        ...
        else if(g_pcRead[0] == 0x02){         //Command Message - take in data, prep response
            ...
            if(g_pcRead[1] = g_ui8ID){
                ...
                xQueueOverwrite(g_pCONTPosQueue, &ui16CmdPos);

                //
                //Write command torque to controller task
                //
                ui8CmdTorque = g_pcRead[4];
                xQueueOverwrite(g_pCONTTorqueQueue, &ui8CmdTorque);
        ...
        vTaskDelayUntil(&ui16LastTime, 1000 * ui32CommsDelay / portTICK_RATE_US);
    }
}
