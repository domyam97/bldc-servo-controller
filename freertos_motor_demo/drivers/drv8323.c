...
DRVConfig(uint32_t ui32PortNum, uint32_t ui32SSIClk, uint32_t ui32BitRate)
{
    ...
    MAP_PWMGenPeriodSet(PWM0_BASE, PWMGEN_A, PWM_GEN_PERIOD); //~30kHz PWM output
    ...
    SetPWM(PWMGEN_A, PWM_FULL);
    SetPWM(PWMGEN_B, PWM_FULL);
    SetPWM(PWMGEN_C, PWM_FULL);
...
{
    if(ui32Width == PWM_FULL){
        ui32Width = PWM_FULL -1;
    }
    if(ui32PWMGen == PWM_GEN_0)
    {
        MAP_PWMPulseWidthSet(PWM0_BASE, PWM_OUT_1, (PWM_FULL-ui32Width)*PWM_GEN_PERIOD/PWM_FULL);
    }
    else if(ui32PWMGen == PWM_GEN_1)
    {
        MAP_PWMPulseWidthSet(PWM0_BASE, PWM_OUT_3, (PWM_FULL-ui32Width)*PWM_GEN_PERIOD/PWM_FULL);
    }
    else if(ui32PWMGen == PWM_GEN_2)
    {
        MAP_PWMPulseWidthSet(PWM0_BASE, PWM_OUT_5, (PWM_FULL-ui32Width)*PWM_GEN_PERIOD/PWM_FULL);
    }
}
...
ProgramDRVSettings(void)
{
    ...
    WriteRegister(0x02, 0x020); //3x PWM Mode

    WriteRegister(0x03, 0x011);

    WriteRegister(0x04, 0x322);

    WriteRegister(0x05, 0x55E);
